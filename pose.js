...
// Add this function to calculate the angle between three points
  ...
  if (results.poseWorldLandmarks && window.unityInstance && sendToUnity) {
    const poseData = {
      landmarks: results.poseWorldLandmarks.map((landmark) => ({
        //Unity sahnesi için x ve y yi - ile çarptık
        x: -landmark.x,
        y: -landmark.y,
        z: landmark.z,
    ...
    // Calculate arm angles if landmarks are detected
    if (results.poseLandmarks) {
      ...
      // Add shoulder angles (abduction/adduction)
      ...
      // Include arm angles in the poseData object
      poseData.armAngles = {
      ...
      // Log arm angles if in debug mode
      if (debugMode) {
        console.log("Arm Angles:", poseData.armAngles)
      }
    }
